-- استئذان — schema, constraints, indexes
-- Run in Supabase SQL Editor (or via scripts/apply-schema.mjs with service role)
...
create table if not exists public.profiles (
  ...
  constraint profiles_identifier_check check (
    (role = 'PARENT' and national_id is not null)
    or (role = 'CLASS_STAFF' and username is not null)
    or (role = 'ADMIN')
  )
);
...
create index if not exists idx_requests_guardian on public.permission_requests (guardian_id);
...
-- Auto-create profile stub is NOT used; profiles are created by admin/seed with auth user.

-- Seed 24 classes if empty
